...
#define ADVANCE_FRAME_PIN 4
#define END_OF_FILM_PIN 5
#define ADVANCE_FRAME_LED 25
#define END_OF_FILM_LED 25
...
#define EDGE_FALL_DEBOUNCE_DELAY_US 8000    ///< Debounce duration for falling edge.
...
// PIO indices
#define ADVANCE_FRAME_PIN_PIO 0
#define END_OF_FILM_PIN_PIO 1
#define ADVANCE_FRAME_LED_PIO 2
#define END_OF_FILM_LED_PIO 3
...
// PIO setup
static PIO pio[2] = {pio0, pio1}; // fixed: array of PIO, not PIO*
 ...
 */
void pio_setup(const pio_program_t *program, PIO pio_inst, uint *sm, uint *offset, uint gpio_base)
{
    // Claim a free PIO and state machine, and load the program for the specified GPIO range
    bool rc = pio_claim_free_sm_and_add_program_for_gpio_range(program, &pio_inst, sm, offset, gpio_base, 1, true);
    hard_assert(rc);                                              // Ensure PIO program setup succeeded
    frame_signal_program_init(pio_inst, *sm, *offset, gpio_base); // Initialize the PIO program
    pio_sm_set_enabled(pio_inst, *sm, true);                      // Enable the state machine
}
...
void gpio_irq_callback_isr(uint gpio, uint32_t event_mask)
{
    if (gpio == ADVANCE_FRAME_PIN)
    {
        ...
        { // If a valid signal is detected
            if (irq_status == 1)
            {
                ...
                pio[0]->txf[sm[1]] = frame_signal_duration;
                ...
                if (edge_fall_alarm_id < 0)
                    printf("Edge fall alarm error %llu\n", edge_fall_alarm_id);
        ...
        else if (event_mask & GPIO_IRQ_EDGE_RISE)
        {
            ...
            {                   // Detect valid rising edge
                ...
                if (edge_rise_alarm_id < 0)
                    printf("Edge rise alarm error %llu\n", edge_rise_alarm_id);
            }
        }
    ...
    else if (gpio == RESET_PIN)
    {
        if (event_mask & GPIO_IRQ_EDGE_FALL)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (pio_sm_is_claimed(pio[i / 2], sm[i]))
                    pio_sm_drain_tx_fifo(pio[i / 2], sm[i]);
            }
...
int main()
{
    ...
    pio_setup(&frame_signal_program, pio[0], &sm[ADVANCE_FRAME_PIN_PIO], &offset[ADVANCE_FRAME_PIN_PIO], PASS_ON_FRAME_ADVANCE_PIN);
    pio_setup(&frame_signal_program, pio[1], &sm[END_OF_FILM_PIN_PIO], &offset[END_OF_FILM_PIN_PIO], PASS_ON_END_OF_FILM_PIN);
    pio_setup(&frame_signal_program, pio[0], &sm[ADVANCE_FRAME_LED_PIO], &offset[ADVANCE_FRAME_LED_PIO], ADVANCE_FRAME_LED);
    pio_setup(&frame_signal_program, pio[1], &sm[END_OF_FILM_LED_PIO], &offset[END_OF_FILM_LED_PIO], END_OF_FILM_LED);
    ...
    while (1)
    {
        tight_loop_contents(); // main loop idle
    }
